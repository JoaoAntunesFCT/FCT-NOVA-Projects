...
int calculaNota(float t1, float t2, float tp1, float tp2){
	...
	if (resto <= ARREDONDA) return (int) nota;
	return (int) (nota+1);
}
int eAprovado(int nota){
	if (nota >= APROVADO) return 1;
	return 0;
}

void lerNotasAluno(int * n,float * t1, float * t2, float * tp1, float * tp2){
	printf("Numero de aluno: ");scanf("%d", n);
	printf("Notas dos testes (dois valores reais): ");scanf("%f %f",t1,t2);
	printf("Notas dos trabalhos (dois valores reais): ");scanf("%f %f",tp1,tp2);
}
